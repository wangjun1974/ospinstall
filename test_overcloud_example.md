...
# （可选）如果 public 所在的网络没有 vlan id 就创建 flat 类型的网络
(overcloud) [stack@undercloud ~]$ openstack network create public \
  --external --provider-physical-network datacentre \
  --provider-network-type flat
 
# 创建 public subnet, 根据 External 的情况 ip 地址分配情况调整 start 和 end
# 在 templates/network_data.yaml 里合理规划 External 的地址范围
# 在 undercloud 查看已占用情况
# (undercloud) [stack@undercloud ~]$ openstack port list | grep 192.168.122 
(overcloud) [stack@undercloud ~]$ openstack subnet create public-subnet \
  --no-dhcp --network public --subnet-range 192.168.122.0/24 \
  --allocation-pool start=192.168.122.100,end=192.168.122.110  \
...
# 转换镜像格式为 raw 格式，ceph 存储要求镜像格式为 raw 格式
